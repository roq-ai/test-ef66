...
import { getEmojiLibraries } from 'apiSdk/emoji-libraries';
import { EmojiLibraryInterface } from 'interfaces/emoji-library';
import { getImageLibraries } from 'apiSdk/image-libraries';
import { ImageLibraryInterface } from 'interfaces/image-library';
import { getGroupChats } from 'apiSdk/group-chats';
import { GroupChatInterface } from 'interfaces/group-chat';

import { EmojiLibraryListPage as ShowCaseList } from 'pages/emoji-libraries';

function DashboardPage() {
  const { session } = useSession();

  const emojiLibraryStat = useSWR<PaginatedInterface<EmojiLibraryInterface>>(
    () => `/emoji-libraries`,
    () => getEmojiLibraries({ limit: 1, offset: 0 }),
  );

  const imageLibraryStat = useSWR<PaginatedInterface<ImageLibraryInterface>>(
    () => `/image-libraries`,
    () => getImageLibraries({ limit: 1, offset: 0 }),
  );

  const groupChatStat = useSWR<PaginatedInterface<GroupChatInterface>>(
    () => `/group-chats`,
    () => getGroupChats({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Emoji Libraries',
      result: emojiLibraryStat?.data?.totalCount,
      isLoading: emojiLibraryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Image Libraries',
      result: imageLibraryStat?.data?.totalCount,
      isLoading: imageLibraryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Group Chats',
      result: groupChatStat?.data?.totalCount,
      isLoading: groupChatStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Emoji Libraries
                </Text>
                <NextLink href={`emoji-libraries`} passHref legacyBehavior>
                  <Button
